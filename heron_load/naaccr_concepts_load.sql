...
select * from tumor_item_value@kumc tiv where 1=0;

-- check installation of I2B2METADATA2 static stuff (from Dustin Key)
select c_basecode from I2B2METADATA2.naaccr_ontology@deid where 1=0;
...
select * from who.topo@kumc where 1=0;
...
from tumor_item_value@kumc tiv;
...
left join naacr.t_code@kumc nc
  on nc.itemid = tiv.ItemID
 and nc.codenbr = tiv.codenbr
left join i2b2metadata2.NAACCR_ONTOLOGY@deid ino
on ino.c_basecode = tiv.concept_cd
*/
from tumor_reg_codes tiv
   , naacr.t_code@kumc nc
   , i2b2metadata2.NAACCR_ONTOLOGY@deid ino
