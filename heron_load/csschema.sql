...
Somewhat obsolete as of NAACCR v18, but still useful for pre 2018 cases.

Copyright (c) 2016-2019 University of Kansas Medical Center
...
create or replace temporary view tumor_cs_schema as
with per_tumor as (
select primarySite as primary_site, histologicTypeIcdO3 as histology
...
select recordId
     , patientIdNumber
     , '@' naaccrId
     , concat('CS|',  sra.cs_schema_name, '|',
              substr(sra.naaccrId, length('csSiteSpecificFactor1')),
              ':', sra.raw_value) concept_cd
     , '@' provider_id
     , sra.start_date
     , '@' modifier_cd  -- IDEA: modifier for synthesized info?
     , 1 instance_num
     , '@' valtype_cd   -- ISSUE: numeric values, e.g. for Breast SSF3 Number of ... Nodes
...
and cs_schema_name is not null
