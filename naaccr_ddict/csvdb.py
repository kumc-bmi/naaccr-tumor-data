...
'''
import csv
from posixpath import join as posix_join, splitext
...
def loadz_db(db, csv_storage, csv_dir_name):
    name, _ext = splitext(csv_dir_name)
    load_db(db, zip_path(csv_storage) / name)


def load_db(dest, src, setup=None):
...
def load_table(dest, table, header, rows,
    ...
    def do_batch():
        ...
        log.info('inserted %s rows into %s',
            len(batch), table)
        del batch[:]
...
def zip_path(backing_store):
    ...
    def zip_open(there, mode='r'):
        ...
        return zbak.open(there, mode=mode)

    def path_exists(there):
        return there in zbak.namelist()
    return Path('', (zip_open, posix_join, path_exists))
...
if __name__ == '__main__':
    def _privileged_main():
        ...
        main(argv, stdout, stderr,
             cwd=Path('.', (io_open, os_path.join, os_path.exists)),
